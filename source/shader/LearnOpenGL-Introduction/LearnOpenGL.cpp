...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			projection = glm::perspective(glm::radians(camera.m_fFov), 800.0f / 600.0f, 0.1f, 100.0f);
...
GLFWwindow* init()
{
	...
	window = glfwCreateWindow(800, 600, "OPENGL INTRO", NULL, NULL);
	if (window == NULL)
	{
...
void texture(Shader* myShader)
{
	...
	PrintFullPath("gitee.png");
	data = stbi_load("gitee.png", &width, &height, &nrChannels, 0);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	data = stbi_load("opengl.png", &width, &height, &nrChannels, 0);
