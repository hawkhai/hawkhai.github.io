...
import cv2
import numpy as np
from .wtools import BGR2YUV, YUV2BGR
...
class Watermark():
    ...
    def init_block_add_index(self, img_shape):
        ...
        # 假设原图长宽均为 2 的整数倍,同时假设水印为 64*64,则 32*32*4
        # 分块并 DCT
    ...
    def read_ori_img(self, filename):
        ...
        if self.color_mod == 'RGB':
            ...
            self.ori_img_YUV = BGR2YUV(ori_img)
        ...
        self.ha_block_shape = (int(self.ha_Y.shape[0]/self.block_shape[0]),
                               int(self.ha_Y.shape[1]/self.block_shape[1]),
                               self.block_shape[0],
                               self.block_shape[1])
        strides = self.ha_Y.itemsize * (np.array([self.ha_Y.shape[1]*self.block_shape[0],
                                                 self.block_shape[1],
                                                 self.ha_Y.shape[1],
                                                 1]))
    ...
    def embed(self, filename):
        ...
        if self.color_mod == 'RGB':
            ...
            embed_img = YUV2BGR(embed_img_YUV)
    ...
    def extract(self, filename, out_wm_name):
        ...
        if self.color_mod == 'RGB':
            ...
            embed_img_YUV = BGR2YUV(embed_img)
        ...
        ha_block_shape = (int(ha_Y.shape[0]/self.block_shape[0]),
                          ...
                          self.block_shape[0],
                          self.block_shape[1])
        strides = ha_Y.itemsize*(np.array([ha_Y.shape[1]*self.block_shape[0],
                                           self.block_shape[1],
                                           ha_Y.shape[1],
                                           1]))
