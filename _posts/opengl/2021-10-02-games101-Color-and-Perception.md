...
**为什么光线会被折射成不同的颜色？**
不同的波长对应不同的折射率；一束光可以用 **光谱（** 光线能量在不同波长上的分布 **）** 来描述。图形学通常关心的是可见光谱，大约分布在 400nm 到 700nm 之间。
...
加色系统可以使用上面的线性组合基本颜色的方式来匹配色域内的颜色；超出这组三原色色域时，就需要引入负系数或换用更大的颜色空间。如下图是一个基本的例子，给你一个目标颜色，希望用 3 种不同的颜色去混合匹配得到相同的颜色
...
CIE 是一个组织，它们定义过 CIE RGB 等颜色匹配系统；在单色光匹配实验里，测试颜色是 **单波长** 的颜色（某种激光），光线的 SPD 近似一个单一的 Δ 函数。
...
RGB 是一类加色颜色模型；sRGB 是其中一个被广泛使用的标准 RGB 色彩空间，规定了基色、白点和传递曲线。

sRGB 的颜色系统广泛应用于各种成像设备（比如电脑显示器）和图像文件交换；不同设备还需要通过校准和色彩管理尽量接近这个标准。
...
如下图，这个色域有一个中心白点；靠近白点的颜色饱和度低，越靠近边界饱和度通常越高，不能简单说白色“最不纯”。
