...
三维中两个向量的叉乘，又叫向量积、外积、叉积，叉乘的运算结果是一个向量而不是一个标量。并且两个向量的叉积与这两个向量组成的坐标平面垂直。
...
在 3D 图形学中，叉乘的概念非常有用，可以通过两个向量的叉乘，生成第三个垂直于 a，b 的法向量，从而构建 X、Y、Z 坐标系。
...
二维向量常用标量 $a_x b_y-a_y b_x$ 表示对应的有符号面积；其绝对值等于由向量 $a$ 和向量 $b$ 构成的平行四边形面积。
