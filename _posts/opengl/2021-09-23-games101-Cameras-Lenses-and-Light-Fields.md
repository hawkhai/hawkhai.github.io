...
最后，当快门打开、光线通过透镜之后，到达传感器（相当于胶片）的光能会被积分记录下来，再通过传感器上的信息得到最终图像：
...
右边相当于传感器平面， **传感器的每一点记录的是到达该点的光线能量积分** ，因此每一点存储的也就是 **irradiance** （如果读者不清楚辐射度量学可以略过，并不影响本文阅读）。
...
(1) 与光轴平行射入理想薄透镜的光会汇聚到焦点

(2) 从焦点出发射入理想薄透镜的光会以平行于光轴的方向射出（光路可逆性）
