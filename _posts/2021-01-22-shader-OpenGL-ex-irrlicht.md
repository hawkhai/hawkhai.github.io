...
开始觉得很难，学了一段时间的 OpenGL，觉得也就那样，尝试改造 3D 引擎，哇靠，一千多个 API，常用的 284 个，而且 OpenGL & OpenGL ES 存在巨大差异，不同硬件之间还存在差异，唏嘘不已，大意了大意了。

OpenGL ES & OpenGL 存在大量 API 交叉的情况，为了兼容，很多 API 采用 wglGetProcAddress 函数指针的形式。分了五类：
