...
标准卷积的一个输出通道对应一个三维卷积核，它会同时覆盖空间维度和所有输入通道；每个输入通道有各自的权重切片，并不是所有通道共用同一张二维过滤器。问题在于，为什么一定要把空间卷积和跨通道混合放在同一步？我们为什么不能每个通道先分开考虑？
...
因此，depthwise separable convolution 通常能比标准卷积显著降低参数量和计算量；效果是否更好取决于网络结构、数据集和训练方式。
**它的关键不是“比标准卷积更细地学习每个通道”，而是先做逐通道空间卷积，再用 1×1 pointwise convolution 做跨通道混合。**
...
经过 channel shuffle 之后，Group conv 输出的特征能和其他组交换信息，缓解组卷积带来的通道隔离。ShuffleNet 单元里通常同时使用 pointwise group convolution、channel shuffle 和 depthwise convolution；不能把 ShuffleNet 里的分组卷积直接说成 depthwise 卷积。
