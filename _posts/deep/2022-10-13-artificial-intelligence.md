...
2. 与距离计算无关的基于树的模型，比如决策树、随机森林等，
    ...
    而与特征间的相对大小无关。
    使用 Z-Score 归一化通常不会改变树模型的切分逻辑，也不保证提高准确率。
    所以可以说归一化对于树型模型不那么重要，
...
K- 最近邻算法（K-Nearest Neighbors，KNN）非常简单。KNN 通过在训练集中搜索 K 个最相似的实例，即 K 个邻居；分类时通常对这些邻居的标签做多数投票，回归时通常对邻居的目标值做平均或加权平均。
...
人工神经网络（Artificial Neural Networks，ANN）可以处理大型复杂的机器学习任务。神经网络本质上是一组带有权值的边和节点组成的相互连接的层，节点通常称为神经元。在输入层和输出层之间，我们可以插入一个或多个隐藏层；隐藏层数量取决于具体模型设计。

人工神经网络的工作原理与大脑的结构类似。一组神经元被赋予一个随机权重，以确定神经元如何处理输入数据。通过对输入数据训练神经网络来学习输入和输出之间的关系。在训练阶段，系统可以访问正确的答案。

如果网络不能准确识别输入，训练算法会根据损失函数调整权重。经过充分且合适的训练后，它可能学到稳定的模式，但不保证对所有样本都始终正确。
...
**ReLu 和 Leaky ReLu**
* ReLU 的输出是 $\max(0,x)$：输入大于 0 时输出原值，小于 0 时输出 0，计算速度快。
...
3. BP 算法用于高效计算梯度，具体参数更新还要结合梯度下降、Adam 等优化器；非凸训练中可能收敛到局部极小值或鞍点。
