...
AlexNet 为什么有 8 个可学习层，为什么第二层就变成 5x5，后面又变成 3x3 了？
...
LSTM 的表现通常比普通时间循环神经网络及隐马尔可夫模型（HMM）更好，比如用在不分段连续手写识别上。
...
**2006 年是深度学习复兴中的标志性节点之一** ，Hinton 在 SCIENCE 上发文，提出一种叫做自动编码机 (Auto-encoder) 的方法，
部分解决了神经网络参数初始化的问题。
...
EM 算法通常只能保证目标函数单调改进并收敛到局部最优或驻点，不能保证全局最优。
梯度下降、启发式方法等也只是求解局部最优问题的常用手段，并不能保证适用于任何局部极值问题。
