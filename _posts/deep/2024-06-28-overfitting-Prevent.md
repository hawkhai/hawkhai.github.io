...
通过上式可以看到，L1 范数就是向量各元素的绝对值之和；把 L1 范数作为正则项时，常被称为 Lasso regularization。那么问题来了，为什么我们希望稀疏化？稀疏化有很多好处，最直接的两个：
* 特征选择
* 可解释性
...
标准化（Standardization）：按均值和标准差把数据缩放到均值约为 0、标准差约为 1；它不保证原始数据变成标准正态分布。标准化公式为：
