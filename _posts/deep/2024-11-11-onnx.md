...
HLOCAL WINAPI MyLocalFree(HLOCAL hMem) {
    ...
    // 不要随手用 HeapFree 替代 LocalFree；LocalAlloc/LocalFree 必须配对。
...
准备输入数据。
float16 底层是 16 bit；ONNX Runtime C++ API 中可用 `Ort::Float16_t` ，或者使用非模板 `CreateTensor` 并显式指定 `ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16` 。不要用 `CreateTensor<uint16_t>` ，否则类型会被推成 UINT16。
```cpp
...
std::vector<Ort::Float16_t> input_data_float16(input_data_float32.size());

for (size_t i = 0; i < input_data_float32.size(); ++i) {
    input_data_float16[i] = Ort::Float16_t(input_data_float32[i]);
}
```
...
处理推理结果。
```
std::vector<Ort::Float16_t> output_data_float16(output_size);
...
for (size_t i = 0; i < output_size; ++i) {
    output_data_float32[i] = static_cast<float>(output_data_float16[i]);
}
```
