...
* $\frac{\partial}{\partial \theta_j} J\left(\theta_0, \theta_1\right)$ 偏导。
...
矩阵求逆常用高斯消元、LU 分解等数值方法，典型复杂度约为 $O(n^3)$。
...
1. reduce number of features（减少特征数量）
    ...
    2. Model selection algorithm（模型选择算法）用特征选择算法进行特征的选择；PCA 属于降维方法，不是直接挑选原始特征。
...
相比 L2 正则化，L1 正则化会产生更稀疏的解，此处稀疏性指的是最优值中的一些参数为 0，和 L2 正则化相比，L1 正则化的稀疏性具有本质的不同。
...
从上面的分析可以看出，k-means 通常会随机初始化 k 个初始聚类中心。而聚类的结果高度依赖质心的初始化。如果初始聚类中心选的不好，k-means 算法最终会收敛到一个局部最优值，而不是全局最优值。为了解决这个问题，引入了 k-means++ 算法，它的基本思想就是：初始的聚类中心之间的相互距离要尽可能的远。而且在计算过程中，我们通常采取的措施是进行不止一次的聚类，每次都初始化不同的中心，以 inertia（簇内平方和）最小的聚类结果作为最终聚类结果。

pyclustering 库下的 kmeans 聚类
