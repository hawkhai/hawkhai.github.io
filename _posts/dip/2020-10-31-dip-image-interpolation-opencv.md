...
| INTER_AREA | 使用像素区域关系进行重采样。它可能是图像缩小时的首选方法，因为它较容易避免摩尔纹；但在图像放大时效果通常不占优，OpenCV 文档中说明它类似于 INTER_NEAREST 方法。 |
...
4. 基于像素区域关系：共分三种情况，图像放大时不一定优于其它插值；图像缩小（x 轴、y 轴同时缩小）又分两种情况，此情况下更容易避免摩尔纹。
