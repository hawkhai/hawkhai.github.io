...
k 空间中的 phase 对空间位置和结构信息更敏感；mag 表示各频率分量的强弱。只保留相位通常仍能看出大致轮廓，只保留幅度而丢掉相位则很难恢复结构。

因此：k 空间中的幅度主要决定频率成分强弱，相位对位置和结构信息非常关键。
...
* 拉普拉斯变换把连续时间信号从时域变换到复频域（s 域），可以处理一类傅里叶变换不易直接处理的信号。
* Z 变换把离散时间信号变换到复平面上的 z 域；在单位圆上取值时才对应离散时间傅里叶变换的频率响应。
