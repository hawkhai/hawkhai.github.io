...
下面展示的是一张 2592x1944（约 500 万像素）图像，在没有经过任何压缩和利用 JPEG 技术后的文件大小：
...
因此，我们可以将图像从 RGB→YCbCr 空间（Y：亮度，Cb：蓝色色度，Cr：红色色度），缩减 Cb 和 Cr 分量的采样数，而对于亮度 Y，则保留 0~255 等级，这个技术就叫做色度下采样，更常见的称呼是色度抽样。
...
我们来具体看看一个例子，下图是一副 8x8 的图，将其 YCbCr 单独分离出来：

{% include image.html url="/assets/images/240825-dip-jpeg/6406.png" caption="YCbCr 空间" %}
{% include image.html url="/assets/images/240825-dip-jpeg/640x.webp" %}

然后我们分别对 Cb 和 Cr 进行下采样操作（例如用 2x2 区域合成一个色度样本）：

{% include image.html url="/assets/images/240825-dip-jpeg/6407.png" caption="对 Cr 进行下采样，Cb 也进行同样的操作" %}

最终得到如下：

{% include image.html url="/assets/images/240825-dip-jpeg/6408.png" caption="下采样的 YCbCr 空间" %}
...
将 DCT 2D 变换得到表格（上面 Encoding 中左边）与量化表格进行逐元素相除并取整，得到右边蓝色表格，
...
JPEG 压缩技术规定了不同 quality factor 下的量化值，QF 越大，量化表格里的数值就越小，其损失也就越小，同时，结合人眼对色彩的敏感要低于亮度，因此，对于 Y 和 CbCr, 定义了不同的量化系数。

{% include image.html url="/assets/images/240825-dip-jpeg/64037.png" %}

为了进一步对数据进行压缩，我们对经过量化后的稀疏矩阵进一步分析，采用 ZigZag 扫描、游程编码和霍夫曼编码组合手段，进一步减少信息存储，如下图所示：
...
* RGB to YCbCr：利用人类视觉系统对色彩的不敏感特性，对 Cb 和 Cr 下采样；
* DCT 2D 变换：利用人类视觉系统对高频的不敏感特性，舍弃部分高频信息，采用 ZigZag 扫描、游程编码和 Huffman 编码技术，进一步减少数据冗余。
