...
如果 np.ix_ 中输入两个列表，则第一个列表存的是待提取元素的行标，第二个列表存的是待提取元素的列标，第一个列表中的每个元素都会遍历第二个列表中的每个值，构成新矩阵的一行元素。
...
矩阵按元素相乘（Hadamard product）：是矩阵各个对应元素相乘，这个时候要求两个矩阵形状可广播，最简单情况是同样大小。

矩阵乘法：矩阵 a 的第 m 行乘以矩阵 b 的第 n 列，各个元素对应相乘然后求和作为第 m 行 n 列元素的值。

对 `np.array` 而言，按元素相乘直接使用 \* 号即可，也可以使用 numpy 库的 multiply 函数；矩阵乘法使用 dot 函数或 `@` 运算符。`np.cross` 是向量叉乘，不是矩阵乘法。
...
array([[19, 22], # dot
       [43, 50]])
array([-4, -4]) # cross：旧版 NumPy 会把每一行当作二维向量，返回对应叉乘的 z 分量；NumPy 2.0 起不推荐二维向量写法
array([[ 5, 12], # multiply
       [21, 32]])
```
...
numpy 库的对象有数组和矩阵，两者看起来长得差不多，但在性质、运算上有很大不同。旧代码可通过 array 函数和 matrix / asmatrix 相互转化；NumPy 2.0 已移除 np.mat，新代码尽量使用 ndarray。

向量：dot 是点乘法，cross 是叉乘，\* 和 multiply 为对应位置相乘。

矩阵 / 二维数组：dot 或 @ 是矩阵乘法，\* 和 multiply 为对应位置相乘；不要把矩阵乘法叫作叉乘。

#### dot 函数

* 对于秩为 1 的数组，执行对应位置相乘，然后再相加，等价于向量的点乘；
* 对于二维数组，执行矩阵乘法运算；更高维时按 NumPy 的批量矩阵乘法规则处理；
...
* 对 `np.matrix`，\* 表示矩阵乘法；但 `np.matrix` 已不推荐在新代码中使用，普通 `ndarray` 请用 `@` 或 `np.matmul` 表示矩阵乘法。
