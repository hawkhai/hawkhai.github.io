...
奇异值分解可以看作是对特征值分解思想的推广：它不要求矩阵是方阵，而是将一个 $$m \times n$$ 的矩阵分解成三个矩阵相乘。
...
如果方阵 A 满足可对角化条件，就可以找到足够多的线性无关特征向量。只有实对称矩阵、正规矩阵这类特殊矩阵，特征向量才通常能取成正交。每一个都满足表达式 $$Av=\lambda v$$。
那么，就可以写成如下形式
...
如果矩阵是正交矩阵，那么对应的变换是保持长度和角度的正交变换，可理解为旋转或反射。
所以，奇异值分解就是将一个线性变换分解成先做一个正交变换，后拉伸 / 压缩，最后再做一个正交变换的过程。
