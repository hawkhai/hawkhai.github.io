...
Gabriel 喇叭会导出一个非常诡异的数学悖论：如果按表面积来计算，给 Gabriel 喇叭的表面刷一遍需要无穷多的涂料；但按体积积分来看，把“理想流体”倒进 Gabriel 喇叭填满内部空间，所需体积反而是有限的。真实油漆有厚度和分子尺度限制，不能把这个比喻当作物理施工方案。
...
好多物理思想可以通过思维实验澄清和推动，但物理定律仍需要数学建模与实验检验。经典的物理思维实验有 Newton 大炮、Galileo 斜塔实验、Schrödinger 的猫猫、Maxwell 的妖怪等等。还有，Turing 机也是一个伟大的思维实验。
