...
帮助大模型建立一个验证的闭环。
* 首先跟它描述需求，关键地方写日志。
* 编译运行，然后把日志文件扔给它，让它分析。
* 它修改了代码，再次编译运行，然后把日志文件再扔给它，让它分析。
* 多重复几轮，大部分问题都能解决。关键的地方要让它加详细的日志。
    * 比如：关注整个视频数据流 及其 相关的核心 变量，及其函数的返回值等。都写入日志。
* 让大模型帮你分析日志的时候，要打开 日志 对应的代码 工程 作为 上下文，它才可以交叉验证。
* 以前是通过调试解决 bug，现在是通过日志解决 bug。
...
大模型写的代码量太大时，逐行 Review 会变得不现实，但不能因此变成无条件信任；更合理的是把验证重点转向测试、接口契约、日志、关键路径审查和可回滚的小步提交。
测试通过，向前走，不回头。

**编译器刚出来的时候，编译器还不成熟、存在 bug、优化能力有限等问题，许多开发者为了确保程序运行正确、高效，会选择查看编译后的汇编代码，甚至手写。但随着项目复杂度和规模的提升，这就变得非常不现实了。后来大家并不是盲目信任编译器，而是通过语言规范、测试、调试器、静态分析和构建流程来建立信任。对于 Vibe Coding，也不应该忘记代码的存在，而是要把关注点从逐行手写扩展到产品验收、关键代码审查、自动化测试和线上质量监控。**
...
- 任务名称：代码清理
- 背景：
  - 脚本文件 font_serv.py 由于不断迭代，文件越来越大，须要清理无用残留函数。很多代码本身也是 AI 生成的，代码已经不熟悉了。
- 提示词：
  - 脚本 font_serv.py 的函数入口是 main，请从这个函数递归分析，然后把没用到的函数直接删除。 **注意：只删除，不做其它任何改动。**
- 提效数据：
  - 3000 行的代码，缩减到 2000 行，几分钟即完成。人工可能要一个小时去大概梳理依赖关系。
- 满意度
  - 非常满意。
- 经验总结（个人感受，优缺点）：
  - AI 写的代码，让 AI 去维护，效果也不错，如果人为介入，须要 Review 的代码量就实在是太大了。
  - 风险：AI 可能自作主张修改代码，这个可以在提示词里面强调一下。

案例（代码翻译）
- 任务名称：代码翻译
- 背景：
  - 训练的模型是 pytorch 版本，不同场景可能切换到 mnn 或者 onnx 推理。
  - 如果自己去写，须要阅读 mnn & onnx 的文档，费时费力。
- 提示词：
  - image_infer.py 是我是实现的 pytorch 版本，请参考实现 mnn 版本和 onnx 版本。 **注意：逻辑要保持绝对一致。**
- 提效数据：
  - 几天的工作，在 AI 的帮助下，几分钟就可以完成。并提供了 image_infer_onnx.py 和 image_infer_mnn.py 版本。
- 满意度
  - 非常满意。
- 经验总结（个人感受，优缺点）：
  - 避免去学习 mnn & onnx 的文档。极大提高了效率。
  - 额外风险，可能翻译出来的代码不对，这种情况就须要用同样的一批样本去测试，测试速度和准确度，给出合理的评估。 **基于单元测试的开发。**
  - **给 AI 的脚本或者任务要相对独立完备，须要的信息都是自洽的，源码文件干净，这样它能工作的更好。**
