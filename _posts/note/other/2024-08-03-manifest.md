...
这个里面就是二进制标示的 manifests 文件。不要手工往 Windows\WinSxS\ 目录硬拷文件；
系统级运行库应通过对应的 Visual C++ Redistributable 或安装包安装。下面的 WinSxS 路径只适合理解机制，
不是推荐的发布方式：
...
所以你想要发布一个程序，比较稳妥的做法是优先使用官方 Visual C++ Redistributable；
如果必须做私有部署，再把 redist 目录下面的相应库文件夹和你的可执行文件放在一起发布。
