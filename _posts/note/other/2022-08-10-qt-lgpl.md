...
4. 需要确保共享对开源许可软件（Qt）所做的所有修改；
    - 若修改 Qt 本身并分发，修改部分通常需要按相应 LGPL/GPL 条款提供源码；不修改 Qt 则重点是允许用户替换/重链接 LGPL 库
...
3. 可以随产品一起提供 Qt 库，但需要同时满足 LGPL 的通知、许可证文本、Qt 源码获取方式以及允许用户替换/重链接等要求；不能简单理解为“随包发布就违反 LGPL”。
...
因为设备不对第三方公开，对生态系统没有任何贡献。LGPLv2.1 没有充分处理这类“锁定设备”问题，自由软件基金会的 LGPLv3/GPLv3 条款加强了用户替换和运行修改版的权利。对用户产品分发 LGPLv3 授权的软件时，通常需要提供足够的安装信息，不能阻止用户运行重链接后的版本。
...
那商业付费什么意思呢？商业许可证可以免去开源许可证下的部分合规义务。静态链接 LGPL 库并不自动等于“必须付费”，但通常需要提供可重链接的目标文件/机制、许可证文本和相关源码获取方式；不想承担这些义务时才常选择商业授权。
...
* BSD 类，如 Apache/BSD/MIT 等：
    * 通常允许商用，但仍需遵守版权声明、许可证文本、NOTICE/专利条款等要求，不能说完全无风险。
* MPL 类，如 MPL/EPL 等：
    * 无修改无需开源，修改部分要开源。
* LGPL：
    * 传染性弱于 GPL。产品对外分发或销售时，需要允许用户替换/重链接 LGPL 库；动态链接通常更容易合规，静态链接则通常要提供目标文件或其它可重链接方式。
* GPL:
...
Qt 公司称，更新的开源许可证将能在用户使用开源版的 Qt 时确保用户自由；LGPLv3/GPLv3 通过安装信息等要求限制“锁定设备”阻止用户运行修改版的做法，
在 LGPLv3 下分发的软件还向软件接收者给予相应专利许可。
