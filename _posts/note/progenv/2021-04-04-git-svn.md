...
```
git reset --hard 9164299c259d654db35e1dea0f903457d3fbcd95
git push origin master --force-with-lease
```
`reset --hard` 会丢弃未保存的工作区修改，`--force-with-lease` 仍然是强推；只在确认目标分支历史需要回滚、且已和协作者同步后使用。
...
拉取，从服务器拉取代码到本地，加 `--rebase` 会先取远端更新，再把本地提交按顺序重放到最新远端提交之后；它不是普通 merge。
