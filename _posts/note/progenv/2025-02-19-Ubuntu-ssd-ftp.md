...
完成这些步骤后，新SSD就可以正常使用了。


# 在Ubuntu上创建FTP服务并指定根目录为 `/mnt/ssd/ftp`，同时支持开机自启动，可以使用 `vsftpd`（Very Secure FTP Daemon）。以下是详细步骤：
...
# 在Ubuntu上使用 `vsftpd` 创建两个FTP账号，一个只读账号，一个支持读写的账号，可以按照以下步骤操作：
...
完成以上步骤后，两个FTP账号即可正常运行，分别支持只读和读写权限。
