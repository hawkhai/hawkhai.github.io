...
原来 64 位平台下只有一种变形的 \__fastcall 的调用约定，前 4 个整数或指针参数先放入 RCX、RDX、R8、R9 寄存器（浮点参数使用 XMM0-XMM3），更多的参数放入栈区。这个时候我们要注意的是，在 64 位下，调用者还要为前 4 个寄存器参数预留 32 字节的 shadow space/home space，方便被调函数在需要时保存这些寄存器参数，也用于变参等场景。
