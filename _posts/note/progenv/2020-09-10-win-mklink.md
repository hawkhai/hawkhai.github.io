...
3. `MKLINK /J Link Target` # 创建指向文件夹的目录联接（Junction）
...
**目录联接只支持文件夹** 。目录联接不管在创建的时候使用的是相对路径还是绝对路径，创建后通常表现为绝对路径。目录联接和符号链接不是一回事；符号链接本身支持文件和文件夹。


## 硬链接

**硬链接是不支持文件夹（目录）的，这和文件系统是有关系的，硬链接和符号链接 / 目录联接的区别就是硬链接本身就是另一个目录项** ，和被指向的文件处在同级语义下，两个文件名指向同一份文件数据，所以删除任意一个，对另外一个都没有影响，而且一个文件更新，另外一个也会同步更新。正因为如此，所以硬链接只能创建在同一个分区中。
...
当目录符号链接使用绝对路径时，很多文件访问场景下看起来接近目录连接点；但二者并不完全一样，重解析标签、远程路径支持、权限要求和解析语义仍有区别。
...
def example():
    ...
    # MKLINK /J Link Target # 创建指向文件夹的目录联接（Junction）
    system("mkdir test")
