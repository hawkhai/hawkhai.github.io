...
* Unicode 由国际标准化组织制定，整合全世界的字符
    ...
    * 码点范围可超过 U+FFFF，具体占用字节数取决于 UTF-8、UTF-16、UTF-32 等编码形式
    * 如果用 UTF-16/UTF-32 只存英文字符，才会相对浪费空间
* UTF(Unicode Transformation Format) 通用转换格式，是 Unicode 的编码形式
...
UTF-8 是一种可变长编码，使用 1-4 字节（8 位）编码所有 Unicode 标量值。与
...
**GBK 和 UTF-8 都是多字节变长编码，但它们都不会在普通非 NUL 字符的编码中包含 `0x00` 字节；所以 `strcpy` 这种以 0（NUL）作为结束符的字节串逻辑通常能截到字符串末尾，但它并不理解字符边界。**
...
* 因为 _beginthreadex 和 _endthreadex 是 CRT 线程函数，所以必须注意运行库配置保持一致；/MD、/MT 都可以，关键是不要在模块之间混用不兼容的 CRT 分配和释放。
* _beginthreadex
...
* 更多的 API：
    ...
    * BOOL SwitchToThread();
* 如果 `main` 返回或调用 `exit` 导致进程结束，进程内其他线程也会被系统终止，不管子线程有没有执行完毕。
