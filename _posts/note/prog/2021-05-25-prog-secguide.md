...
* **auto_ptr** **不再推荐** 。赋值转移后，指针会置空。因为 auto_ptr 有拷贝语义，拷贝后原对象变得无效，再次访问原对象时会导致程序崩溃；
    * auto_ptr 不能管理数组，因为其实现中调用的是 delete 而非 delete[]；现代 C++ 应使用 `std::unique_ptr<T[]>` 或容器管理数组。
* **unique_ptr** unique_ptr 则禁止了拷贝语义，但提供了移动语义，即可以使用 std::move() 进行控制权限的转移。
  ...
  string* raw = upt1.release(); // 释放所有权但不释放对象；调用方必须接管 raw 并负责 delete
...
void test1() {
    ...
    delete p; // 错误示例：可能看起来内存降低了，但 new[]/delete 混用仍是未定义行为
    p = NULL;
}
...
2. str() 不会附加字符串终止符号（’\0’）。
3. std::ostrstream::str() 返回的缓冲区未必以 ’\0’ 结尾；现代 std::string 的 data()/c_str() 另当别论，C++11 起会提供以空字符结尾的字符数组。
