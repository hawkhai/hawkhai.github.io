...
严格说，下面这个 C++98 兼容实现不是 C++11 `std::unique_ptr` 的完整等价物：它用拷贝构造模拟所有权转移，更接近 `auto_ptr` 的风险模型，不适合放进标准容器；现代 C++ 应删除拷贝、提供移动构造/移动赋值。
...
下面的实现只是 C++98 环境下的简化学习版：引用计数不是原子的，不具备标准 `std::shared_ptr` 对控制块操作的线程安全语义，也没有完整的删除器/弱引用能力。
