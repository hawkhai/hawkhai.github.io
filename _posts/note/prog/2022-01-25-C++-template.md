...
```cpp
int a = (1, 0); // a == 0, 逗号运算符就是取最后一个表达式值为返回结果
int b[] = { (1, 0), (2, 0)} // b <==> {0, 0}
...
可以看出模板参数包中的省略号是包展开，不是逗号运算符本身；这里逗号运算符只是把每次 `_packOne` 调用的结果统一成 0，方便放进数组初始化列表并保证调用发生。我暂没有用到使用省略号错误的情况，且用且注意吧，除了在函数中或者类中使用省略号，
