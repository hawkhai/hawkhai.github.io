...
* 工作集 – 当前驻留在 RAM 中、可不触发缺页就访问的页面集合
* 提交大小 - 进程已提交的私有虚拟内存，可能在 RAM 中，也可能在页面文件中

* 对于 32 位系统，CPU 可直接寻址 2^32=4GB 空间，这也是每个进程的地址空间大小
* 高 2GB 供内核模块使用，称为内核空间。低 2GB 供用户态模块使用，称为用户空间
* 使用 /3GB 开关启动，可以将用户空间增大到 3GB

* 32 位进程通常有 4GB 虚拟地址空间；在默认 32 位 Windows 上，用户态代码通常只能访问 2GB 用户空间
* 各进程有独立的用户态地址空间；内核空间在系统范围内映射，用户态不能直接访问

使用 LargeAddressAware 扩展程序内存地址空间
{% include image.html url="/assets/images/210423-win-memory/20230731-144153.jpg" %}
当然这种方法的缺点：
1. 对 32 位程序来说，即使在 64 位系统上也没办法使用超过 4GB 的用户地址空间；不过以前只能用 2GB，聊胜于无了；
2. 在 32 位 Windows 上，LargeAddressAware 需要配合 /3GB 等启动配置才可能让用户态地址空间超过 2GB；在 64 位 Windows 上运行 32 位 LAA 程序则通常可获得接近 4GB 的用户地址空间
