...
**OpenCV 传统 GPU/ 异构优化里大量使用 OpenCL（尤其是 UMat/T-API 路线），但并不是所有 GPU 相关实现都靠 OpenCL。**
...
OpenCL 图片对象常见支持 RGBA/BGRA/R 单通道等格式；RGB/R8G8B8 这类 3 通道格式在 OpenCL 中并不是普遍保证支持，实际要以 `clGetSupportedImageFormats()` 查询结果为准。
```cpp
...
CL_MEM_ALLOC_HOST_PTR 是这类场景中常用的减少显式复制的方法之一。
对于其他标志，如 CL_MEM_USE_HOST_PTR 或 CL_MEM_COPY_HOST_PTR，驱动程序是否额外复制取决于平台、对齐方式和内存可访问性，不能一概而论。
