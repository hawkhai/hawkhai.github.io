...
1. `0`-`W` 一共 59 个字符用于编码，`XYZ` 作为扩展标记。
    * 可以编码 $0-58$，编码分别为：`0`, `1`... `W`。
2. `X` 表示进位，基准为 59。
    * 可以编码 $59-117$，编码分别为：`X0`, `X1`... `XW`。
3. `Y` 表示两位，基准为 118。
    * 可以编码 $118-3598$，编码分别为：`Y00`, `Y01`... `YWW`。
4. `Z` 表示三位，最高位递归处理，基准为 3599。
    * 可以编码 $3599-∞$，编码分别为：`Z000`, `Z001`... `ZWWW`, `Z00X0`...
...
int b62base64::BBEncode(std::wstring& retv, unsigned long value) {
    ...
    else if ((value - 59 * 2) / 59 < 59) {
        value -= 59 * 2;
        retv.push_back(L'Y');
        retv.push_back(B62_BASE64[value % 59]);
        retv.push_back(B62_BASE64[value / 59]);
        len += 3;
    }
    else {
        const int basek = 59 * 2 + 59 * 59;
        assert(value >= basek); // 不能出现负数。
        value -= basek;

        retv.push_back(L'Z');
        retv.push_back(B62_BASE64[value % 59]);
        retv.push_back(B62_BASE64[value / 59 % 59]);
        len += 3 + BBEncode(retv, value / 59 / 59); // 递归最高位置。
    }
    return len;
}
```
...
unsigned long b62base64::BBDecode(const wchar_t* ptr, unsigned int& index) {
    ...
    if (op == L'X') {
        wchar_t X = ptr[index++];
        return 59 + BBDecode(X);
    }
    else if (op == L'Y') {
        wchar_t Y1 = ptr[index++];
        wchar_t Y2 = ptr[index++];
        return 59 * 2 + BBDecode(Y1) + BBDecode(Y2) * 59;
    }
    else if (op == L'Z') {
        ...
        const int basek = 59 * 2 + 59 * 59;
        return basek + BBDecode(Z1) + BBDecode(Z2) * 59 + Z3 * 59 * 59;
    }
    else {
        return BBDecode(op);
    }
